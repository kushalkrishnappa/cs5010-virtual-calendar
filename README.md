...
- [New Features (Assignment 5)](#new-features-assignment-5)
...
The application can be run in interactive and headless modes as explained later.

With the latest updates, it now supports multiple calendars with timezone management, allowing users to create separate calendars for different purposes (e.g., work, personal) and manage events across different timezones.

## New Features (Assignment 5)
...
2. Execute the jar file using: 
    ```bash
    java -jar cs5010-virtual-calendar.jar --mode [interactive|headless filename]
    ```
    where `interactive` and `headless` are the modes of application usage.
